...
export function messageRow(row, state, observedAt) {
  ...
  // Nested pagination still cannot be certified as FULL attachment recovery, but dropping the
  // message hid the customer's photo entirely (149 threads blocked, agents saw nothing to answer).
  // Store what Meta returned and mark it partial, so the first attachments show and nothing claims completeness.
  const attachmentsTruncated = Boolean(row.attachments?.paging?.next)
  ...
  return { mid: row.id, page_id: state.pageId, psid: state.psid,
    ...
    raw: { _akmez_history: { version: 1, source: 'meta_graph', observedAt, graphVersion: state.graphVersion, attachmentsTruncated }, message: graph } }
}
