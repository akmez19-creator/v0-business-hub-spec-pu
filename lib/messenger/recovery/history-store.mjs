...
export class PgHistoryStore {
  ...
  /**
   * renew: false      - insert only; an existing job is left exactly as it is.
   * renew: 'complete' - a finished recent job older than one cycle runs again.
   * renew: 'evidence' - Meta reported activity newer than what we hold: a finished
   *                     OR blocked job runs again, so one transient Graph failure
   *                     cannot take a thread out of recovery for good.
   */
  async ensureOne(state, renew = 'complete') {
    ...
    if (found.rows.length) {
      ...
      const renewable = renew === 'evidence' ? old.status === 'complete' || old.status === 'blocked' : renew === 'complete' && old.status === 'complete'
      const canRenew = state.mode === 'recent' && renewable && Date.parse(state.startedAt) - Date.parse(old.startedAt) >= this.recentCycleMs
  ...
  async seed(states, { renew = 'complete' } = {}) {
    if (states.length > 250) fail('seed_budget_exceeded')
    await this.transaction(async () => { for (const state of states) await this.ensureOne(validateState(state), renew) })
  }
  /** Newest stored message time per conversation, keyed by both Graph conversation id and psid. Read only. */
  async knownActivity(pageId, conversations) {
    if (!APPROVED_PAGES.includes(pageId)) fail('page_not_approved')
    if (!conversations.length) return new Map()
    return this.transaction(async () => {
      const result = await this.client.query(`SELECT psid,conversation_id,last_message_at FROM messenger_conversations
        WHERE page_id=$1 AND last_message_at IS NOT NULL AND (psid = ANY($2::text[]) OR conversation_id = ANY($3::text[]))`,
      [pageId, conversations.map(c => c.psid), conversations.map(c => c.id)])
      const known = new Map()
      for (const row of result.rows) {
        const at = new Date(row.last_message_at).getTime()
        if (!Number.isFinite(at)) continue
        known.set(row.psid, Math.max(known.get(row.psid) ?? 0, at))
        if (row.conversation_id) known.set(row.conversation_id, Math.max(known.get(row.conversation_id) ?? 0, at))
      }
      return known
    }, true)
  }
  ...
  async commitPage(claim, { messages = [], conversations = [], jobs = [], next, renew = 'complete' }) {
    let newState, stats
    await this.transaction(async () => {
      ...
      if (!this.dryRun) {
        for (const state of jobs) await this.ensureOne(state, renew)
    ...
    if (this.dryRun) {
      for (const job of jobs) {
        ...
        const reopened = renew === 'evidence' && previous && (previous.status === 'complete' || previous.status === 'blocked')
        if (!previous || reopened) this.previewJobs.set(key, clone(job))
      }
