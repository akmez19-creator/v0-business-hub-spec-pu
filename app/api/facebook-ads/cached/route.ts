...
export async function GET(request: Request) {
  ...
  try {
    ...
    // Fetch campaigns and spend for each account (today's data)
    const allCampaigns: unknown[] = []
    const accountSpends: Record<string, number> = {}
    
    for (const account of accounts) {
      try {
        // Get account-level spend for today
        const spendResponse = await fetch(
          `${FACEBOOK_GRAPH_URL}/${account.id}/insights?fields=spend&date_preset=today&access_token=${accessToken}`
        )
        const spendData = await spendResponse.json()
        accountSpends[account.id] = parseFloat(spendData.data?.[0]?.spend || '0')
        
        // Get all campaigns with pagination
        let nextUrl: string | null = `${FACEBOOK_GRAPH_URL}/${account.id}/campaigns?fields=id,name,status,objective,created_time&access_token=${accessToken}&limit=500`
        ...
        while (nextUrl) {
          ...
          const campaignsWithData = campaignsData.data || []
          
          for (const campaign of campaignsWithData) {
            try {
              const insightsResponse = await fetch(
                `${FACEBOOK_GRAPH_URL}/${campaign.id}/insights?fields=spend&date_preset=today&access_token=${accessToken}`
              )
