  ...
  useEffect(() => {
    ...
    if (showPoles && mb) filteredRegions.forEach(r => {
      ...
      el.innerHTML = `
        ...
        <button class="edit-pole-btn" data-locality="${r.locality}" data-lat="${r.lat}" data-lng="${r.lng}" style="width:18px;height:18px;border-radius:50%;background:rgba(34,211,238,0.25);border:1px solid rgba(34,211,238,0.5);display:flex;align-items:center;justify-content:center;cursor:pointer;margin-top:2px;box-shadow:0 2px 6px rgba(0,0,0,0.4);">
                              ...
                              <div key={d.id} className="client-card client-card-need overflow-hidden">
                                ...
                                {isExpandedCard && (
                                  <div className="px-3 pb-3 pt-2 border-t border-white/[0.03] space-y-2">
                                    ...
                                    {/* Status actions row - show different UI if pending review */}
                                    {d.pendingModificationId ? (
                                      <div className="flex flex-col gap-2">
                                        <div className="flex items-center gap-2 p-2 rounded-lg bg-purple-500/10 border border-purple-400/20">
                                          <div className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
      ...
      {placingPin && (
        <>
          ...
          <div className="absolute top-3 left-3 right-3 z-40 flex flex-col gap-2">
            ...
            {/* Compact header: name badge + search in one row */}
            <div className="flex items-center gap-2">
              <div className="flex-shrink-0 bg-cyan-500/20 backdrop-blur-xl border border-cyan-400/30 rounded-lg px-2 py-1 pointer-events-none">
                <div className="flex items-center gap-1.5">
                  <div className="w-1.5 h-1.5 rounded-full bg-cyan-400 animate-pulse" />
                  <p className="text-[9px] font-bold text-white truncate max-w-[80px]">{placingPin.customerName}</p>
                </div>
              </div>
              <div className="relative flex-1">
                <div className="flex items-center gap-2 bg-black/80 backdrop-blur-xl border border-white/20 rounded-lg px-2.5 py-1.5">
...
{/* Search Results Dropdown */}
                {streetResults.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-black/95 backdrop-blur-xl border border-white/20 rounded-xl overflow-hidden max-h-[180px] overflow-y-auto z-50">
                    {streetResults.map((result, idx) => (
                      <button
                        key={idx}
                        onClick={() => selectStreetResult(result)}
                        className="w-full px-3 py-2 text-left hover:bg-white/10 active:bg-white/20 transition border-b border-white/5 last:border-0"
                      >
                        <p className="text-xs font-semibold text-white truncate">{result.text}</p>
                        <p className="text-[9px] text-white/50 truncate">{result.place_name}</p>
                      </button>
                    ))}
                  </div>
                )}
                ...
                {noStreetResults && streetSearch.length >= 2 && !streetSearching && (
                  <div className="absolute top-full left-0 right-0 mt-1 bg-black/95 backdrop-blur-xl border border-white/20 rounded-xl p-2 z-50">
                    <p className="text-xs text-white/50 text-center">No results for "{streetSearch}"</p>
                  </div>
                )}
              </div>
            </div>
          </div>
          <div className="absolute bottom-36 left-3 right-3 z-40 flex items-center gap-2">
            {/* Compact action row - all in one line */}
            <button onClick={() => { setPlacingPin(null); setStreetSearch(''); setStreetResults([]) }}
              className="h-10 px-4 flex items-center justify-center gap-1.5 rounded-lg bg-zinc-800/90 backdrop-blur-xl border border-white/10 text-white/60 font-semibold text-xs active:scale-95 transition">
              <X className="w-3.5 h-3.5" /> Cancel
            </button>
            <button onClick={confirmPinPlacement} disabled={savingPin}
              className="flex-1 h-10 flex items-center justify-center gap-1.5 rounded-lg bg-cyan-500/20 backdrop-blur-xl border border-cyan-400/30 text-cyan-400 font-bold text-sm active:scale-95 transition disabled:opacity-50">
              {savingPin ? <div className="w-3.5 h-3.5 border-2 border-cyan-400/30 border-t-cyan-400 rounded-full animate-spin" /> : <Check className="w-4 h-4" />}
              Confirm Pin
            </button>
            <button onClick={() => {
              const map = mapRef.current
              ...
              className="h-10 px-3 flex items-center justify-center rounded-lg bg-green-500/10 border border-green-400/20 text-green-400 active:scale-95 transition">
              <ExternalLink className="w-4 h-4" />
            </button>
          </div>
        </>
      ...
      {addingPoi && (
        <div className="absolute inset-x-0 bottom-0 z-[60] flex justify-center p-3 pointer-events-none">
          <div onClick={e => e.stopPropagation()} className="pointer-events-auto w-full max-w-[400px] rounded-2xl overflow-hidden bg-[#1a1a2e]/95 backdrop-blur-md border border-amber-500/40 shadow-2xl">
            <div className="flex items-center gap-2.5 p-3 bg-amber-500/20 border-b border-amber-500/20">
              <MapPin className="w-5 h-5 text-amber-400" />
              <div className="flex-1">
                <div className="font-bold text-white text-sm">Add Location</div>
                <div className="text-[10px] text-white/60">Move map to position the pin, then save</div>
              </div>
              <button onClick={() => { setAddingPoi(false); setNewPoiCoords(null); setNewPoiName(''); setNewPoiCategory('landmark') }} 
                className="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-white/60 hover:bg-white/20">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-3 space-y-3">
              <div className="flex gap-2">
                <input type="text" value={newPoiName} onChange={e => setNewPoiName(e.target.value)}
                  ...
                  className="flex-1 px-3 py-2 rounded-lg bg-white/10 border border-white/10 text-white text-sm placeholder:text-white/40 focus:outline-none focus:border-amber-400/50" />
                <select value={newPoiCategory} onChange={e => setNewPoiCategory(e.target.value)}
                  className="w-28 px-2 py-2 rounded-lg bg-white/10 border border-white/10 text-white text-xs focus:outline-none focus:border-amber-400/50">
