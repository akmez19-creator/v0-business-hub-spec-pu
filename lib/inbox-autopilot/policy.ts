...
export function buildReplyPlan(decision: ReplyDecision, context: TrustedContext, products: CatalogueProduct[], deliveryDate: string, now = new Date()): ReplyPlan {
  ...
  if (decision.intent === 'complaint') return stop('customer_issue')
  if (decision.intent === 'change_order') return stop('exchange_or_change_request')
  ...
  const evidence:Record<string,string> = {productId:product.id,productMessageId:pe!.messageId}
