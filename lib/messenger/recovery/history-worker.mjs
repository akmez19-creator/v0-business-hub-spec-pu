...
export class GraphHistoryReader {
  ...
  async get(url, token) {
    ...
    if (!response.ok || json?.error) {
      ...
      // Codes 1/2 are Meta's "unknown/transient, try again" answered as HTTP 400. Treating them as
      // permanent stranded 344 threads whose customers kept writing where nobody could see them.
      const transient = [1,2].includes(code)
      ...
      fail(rate ? 'graph_rate_limited' : 'graph_request_failed', { code, retryable: rate || transient || response.status >= 500 || json?.error?.is_transient === true, delayMs })
    }
    return json
  }
  async read(state) {
